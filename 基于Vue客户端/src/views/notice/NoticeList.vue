...
<script>
    ...
    export default {
        ...
        data() {
            return {
                noticeList: [],  //界面需要展示的站内通知列表数据
                title: "", //标题查询关键字

                addTime: "",  //发布时间
                showAddTimePicker: false, //是否显示选择发布时间Picker
                picker_addTime: new Date(), //发布时间Picker绑定的日期

                pullFlag: true,  //是否允许下拉刷新标志
                page: 1, //默认显示结果第1页
                showQueryNoticeForm: false, //是否显示查询站内通知表单
                isReturnTop: false,
            }
        },
        ...
        methods: {
            ...
            noticeAdd() {
                this.$router.push('/notice/add');  //打开站内通知添加页面
            },
            ...
            noticeDetailClick(noticeId) {
                this.$router.push('/notice/detail/' + noticeId); //跳转到站内通知详情页
            },
            noticeEditClick(noticeId) {
                this.$router.push('/notice/modify/' + noticeId); //跳转到站内通知修改页
            },

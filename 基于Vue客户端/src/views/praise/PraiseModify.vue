...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.praiseId = this.$route.params.praiseId;  //接收表扬id参数
            //向后端请求单条表扬记录
            getPraise(this.praiseId).then(res => {
                var praise = res.data;
                this.praiseId = praise.praiseId; //表扬id
                this.lostFoundObj_lostFoundId = praise.lostFoundObj.lostFoundId; //招领信息
                this.lostFoundObj_title = praise.lostFoundObj.title;
                this.title = praise.title;  //标题
                this.contents = praise.contents;  //表扬内容
                this.addTime = praise.addTime; //表扬时间

...
public class UserInfoController extends BaseController {
	...
	public void list(String user_name,@ModelAttribute("areaObj") Area areaObj,String name,String birthday,String telephone,String regTime,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, areaObj, name, birthday, telephone, regTime, page);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, areaObj, name, birthday, telephone, regTime);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String user_name,@ModelAttribute("areaObj") Area areaObj,String name,String birthday,String telephone,String regTime,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (telephone == null) telephone = "";
		if (regTime == null) regTime = "";
		List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name, areaObj, name, birthday, telephone, regTime, currentPage);
	    /*计算总的页数和总的记录数*/
	    userInfoService.queryTotalPageAndRecordNumber(user_name, areaObj, name, birthday, telephone, regTime);
	    /*获取到总的页码数目*/
	    int totalPage = userInfoService.getTotalPage();
	...
	public void OutToExcel(String user_name,@ModelAttribute("areaObj") Area areaObj,String name,String birthday,String telephone,String regTime, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(user_name == null) user_name = "";
        if(name == null) name = "";
        if(birthday == null) birthday = "";
        if(telephone == null) telephone = "";
        if(regTime == null) regTime = "";
        List<UserInfo> userInfoList = userInfoService.queryUserInfo(user_name,areaObj,name,birthday,telephone,regTime);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "UserInfo信息记录"; 
        String[] headers = { "手机账号","所在学院","姓名","性别","学生照片","出生日期","联系电话","注册时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<userInfoList.size();i++) {
        	UserInfo userInfo = userInfoList.get(i); 
        	dataset.add(new String[]{userInfo.getUser_name(),userInfo.getAreaObj().getAreaName(),userInfo.getName(),userInfo.getSex(),userInfo.getUserPhoto(),userInfo.getBirthday(),userInfo.getTelephone(),userInfo.getRegTime()});
        }
        /*

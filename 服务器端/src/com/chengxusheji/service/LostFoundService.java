...
public class LostFoundService {
    ...
    /*按照查询条件分页查询失物招领记录*/
    public ArrayList<LostFound> queryLostFound(String title,String goodsName,String pickUpTime,String pickUpPlace,String connectPerson,String phone,UserInfo userObj,int currentPage) throws Exception { 
    	...
    	if(null != userObj &&  userObj.getUser_name() != null  && !userObj.getUser_name().equals(""))  where += " and t_lostFound.userObj='" + userObj.getUser_name() + "'";
    	int startIndex = (currentPage-1) * this.rows;
    	return lostFoundMapper.queryLostFound(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<LostFound> queryLostFound(String title,String goodsName,String pickUpTime,String pickUpPlace,String connectPerson,String phone,UserInfo userObj) throws Exception  { 
     	String where = "where 1=1";
    	if(!title.equals("")) where = where + " and t_lostFound.title like '%" + title + "%'";
    	if(!goodsName.equals("")) where = where + " and t_lostFound.goodsName like '%" + goodsName + "%'";
    	if(!pickUpTime.equals("")) where = where + " and t_lostFound.pickUpTime like '%" + pickUpTime + "%'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(String title,String goodsName,String pickUpTime,String pickUpPlace,String connectPerson,String phone,UserInfo userObj) throws Exception {
     	String where = "where 1=1";
    	if(!title.equals("")) where = where + " and t_lostFound.title like '%" + title + "%'";
    	if(!goodsName.equals("")) where = where + " and t_lostFound.goodsName like '%" + goodsName + "%'";
    	if(!pickUpTime.equals("")) where = where + " and t_lostFound.pickUpTime like '%" + pickUpTime + "%'";
